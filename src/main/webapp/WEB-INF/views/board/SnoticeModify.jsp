							...
							<c:forEach items="${b.imgList}" var="attachment" varStatus="i">
								<c:choose>
									<c:when test="${attachment.imgLevel==0}">
										<c:set var="img0" value="${contextPath}/resources/images/board/N/${attachment.changeName }"/>																									
										<c:set var="img0no" value="${attachment.fileNo}"/>
									</c:when>
									<c:when test="${attachment.imgLevel==1}">
										<c:set var="img1" value="${contextPath}/resources/images/board/N/${attachment.changeName }"/>										
